        ...
        for item in self.node_items:
            ...
            if isinstance(item, tuple) or isinstance(item, list):
                if (
                    ...
                    submenu_name = item[0]
                    submenu_id = f"MATERIALIZE_NODES_MT_{submenu_name}" + self.tree_type
...
def materialize_nodes_templatemenu_append(self, context):
    layout = self.layout
    layout.separator()
    layout.menu(
        "MATERIALIZE_NODES_MT_textemplate",
        text="Materialize Nodes Scripts",
    )
    return None


def materialize_nodes_addmenu_append(
    self,
    context,
):
    tree_type = context.space_data.tree_type
    menu_id = f"MATERIALIZE_NODES_MT_{tree_type}"
    self.layout.menu(menu_id)
    return None


def materialize_nodes_nodemenu_append(self, context):
    layout = self.layout
    layout.separator()
    layout.operator(
        "materialize_nodes.node_purge_unused",
...
DRAWFUNCS = (
    materialize_nodes_addmenu_append,
    materialize_nodes_nodemenu_append,
    materialize_nodes_templatemenu_append,
)
...
def register_submenus(custom_nodes, shadertype):
    ...
    def create_submenu(menu_name, user_name, menu_items):
        ...
        menu_cls = type(
            f"MATERIALIZE_NODES_MT_{menu_name}",
            (DynaMenu, bpy.types.Menu),
            {
                "bl_idname": f"MATERIALIZE_NODES_MT_{menu_name}",
                "bl_label": user_name,
...
def append_menus():
    ...
    for cls in {
        MATERIALIZE_NODES_MT_GeometryNodeTree
    }:
